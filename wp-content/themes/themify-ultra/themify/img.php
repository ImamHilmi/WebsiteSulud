...
function themify_get_placeholder($url,$base=false){
	...
	if(!is_file($destination)){
		...
		if(!file_put_contents($destination,$svg,LOCK_EX)){
			return false;
		}
	}
	if($base===true){
...
function themify_createWebp($url){
	...
	if(!empty($available)){
	    ...
	    if(isset($available['Imagick'])){
			...
			if(!isset($lowerExt[1]) || $lowerExt[1]==='webp' || $lowerExt[1]==='bmp' || $lowerExt[1]==='apng'){
				unset($im);
				return $url;
			}
			$lowerExt=$lowerExt[1];
			...
			$webp = file_put_contents($resUrl, $imageBlob,LOCK_EX);
			if($webp){
				$res=$resUrl;
			}
	    }
...
function themify_get_image_size($url,$isLocal=false){
	...
	clearstatcache();
	$dir=rtrim($dir['basedir'],'/');
	if(is_dir($dir.'/tf_images_data/')){//remove old dir
		Themify_Filesystem::delete($dir.'/tf_images_data/');
	}
	$dir.='/tf_images_sizes/';
	if (is_dir($dir)) {
		if(($handle = opendir($dir))){
			...
			while (false !== ($entry = readdir($handle))) {
				if ($entry !=='.' && $entry !== '..' && is_file($dir.$entry) && ($data=file_get_contents($dir.$entry))) {
					$start = strpos($data,$k);
					
					if($start!==false){
						$start+=$len;
						for($i=1;$i<30;++$i){//30 is max length that an image size can be
