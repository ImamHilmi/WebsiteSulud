...
(function ($, Themify, document, window) {
    ...
    const ThemifyGallery = {
        ...
        lightboxSelector(e) {
            ...
            let targetItems,
                index = $swiper_slider.length>0?$swiper_slider[0].swiper.realIndex:($groupItems.length > 1 ? $groupItems.index(this) : 0),
            ...
            if ($groupItems.length > 1 && index !== -1) {
                ...
                $groupItems.each(function (i, el) {
                    const title = $(el).find('img').prop('title'),
                        t = typeof title !== 'undefined' && title !== '' ? title : el.getAttribute('title');
                    targetItems.push({
