...
function themify_search_in_category_filter( $query ) {
	if ( $query->is_search && ! is_admin() && $query->is_main_query() ) {
	    remove_filter( 'pre_get_posts', 'themify_search_in_category_filter', 999 );
...
function themify_feed_settings_action($query){
	...
	if( $query->is_feed ) {
	    ...
	    remove_filter('pre_get_posts','themify_feed_settings_action');
	}
...
function themify_archive_post_order( $query ) {
	if ( $query->is_main_query()) {
		remove_filter( 'pre_get_posts', 'themify_archive_post_order' );
