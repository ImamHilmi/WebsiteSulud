...
(function ($, Themify, document, window, themifyScript) {
    ...
    const updateFullPage = function(options){
        ...
        createFullScrolling = function(options){
			...
			/* row padding is applied to .row_inner element */
			for ( let i = 0, len = rows.length; i < len; ++ i ) {
				let inner = items[i].getElementsByClassName( 'row_inner' ),
					paddingTop = $( rows[i] ).css('padding-top').replace(/%/g, 'vh'),
					paddingBottom = $( rows[i] ).css('padding-bottom').replace(/%/g, 'vh');
				inner[0].style['paddingTop'] = paddingTop;
				inner[0].style['paddingBottom'] = paddingBottom;
				rows[i].style['paddingTop'] = rows[i].style['paddingBottom'] = 0;
			}
