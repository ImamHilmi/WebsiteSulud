...
class Themify_WC{
	...
	public static function before_init(){
		...
		if ( ! is_admin() && themify_check( 'setting-hide_shop_products', true ) ) {
			add_action( 'pre_get_posts', array( __CLASS__, 'hide_shop_products' ) );
		}
	}
	...
	public static function loop_image($image, $product, $size, $attr, $placeholder, $orig_image){
        ...
        if($wc_shortcode || $themify->hide_image!=='yes'){
	...
	*/
	...
	public static function add_to_cart_fragments( $fragments ) {
		...
		$fragments['#cart-icon span'] =$count===0?'<span></span>': sprintf( '<span>%s</span>', $count );
		return $fragments;
	}
	...
	public static function getLoopImageSize(){
		if('custom' === themify_get( 'setting-product_shop_image_size',false,true )){
		    $width=themify_get('setting_product_shop_image_size_woocommerce',false,true);
	...
	public static function hide_shop_products( $q){
		if ( $q->is_main_query() &&  $q->is_post_type_archive() && ! is_admin() && themify_is_shop() ) {
		    $q->set( 'post__in', array( 0 ) );
		    remove_action( 'pre_get_posts', array( __CLASS__, 'hide_shop_products' ) );
